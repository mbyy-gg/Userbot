...
case "deladmin": {
  ...
  break;
        ...
        case "payment": {
          const caption =
`*Payment Pembayaran Skyzopedia 🕊️*
...
`;
          ...
          return ctx.answerInlineQuery([{
            ...
            reply_markup: { inline_keyboard: [[
              { text: "💬 Join Channel Skyzopedia", url: config.channelLink }
            ]] }
          }], { cache_time: 0 });
        }
...
${lastTransactions}`;
    ...
    const scriptButtons = scriptsList.map(s => [
        {
            text: `📂 ${escapeMarkdown(s.name)} - Rp${s.price}`,
...
bot.action("buydo", async (ctx) => {
    ...
    await ctx.reply(
        "*Pilih Kategori Akun Digital Ocean:*",
        {
            parse_mode: "Markdown",
            reply_markup: { inline_keyboard: categoryButtons }
        }
...
bot.action("buyvps", async (ctx) => {
    ...
    const packageButtons = vpsPackages.map(pkg => [
        {
            text: `${escapeMarkdown(pkg.label)} - Rp${toRupiah(pkg.price)}`,
            callback_data: `vps_step1|${pkg.key}`
        }
    ])

    await ctx.reply(
        "*Pilih Paket Ram & Cpu Vps:*",
        {
            parse_mode: "Markdown",
    ...
    bot.action("buypanel", async (ctx) => {
        await ctx.answerCbQuery();
        return ctx.reply(`Ketik ${config.prefix}buypanel username untuk membeli panel.`);
    });
  ...
  inline_keyboard: [
    ...
    bot.action(/do_category_buy\|(.+)/, async (ctx) => {
        ...
        return ctx.editMessageText(`📦 Kategori DO: *${escapeMarkdown(category.toUpperCase())}*\n\n*Pilih Stock Akun:*`, {
            parse_mode: "Markdown",
...
└ Index: ${itemIndex + 1}/${doData[category].length} (kategori)`;
    ...
    bot.action(/back_to_packages/, async (ctx) => {
        ...
        return ctx.editMessageText("*Pilih Paket Ram & Cpu Vps:*", {
            parse_mode: "Markdown",
            reply_markup: { inline_keyboard: packageButtons }
        });
    });
    ...
    bot.action(/vps_step1\|(.+)/, async (ctx) => {
        ...
        if (!vpsSpecs[specKey]) {
            return ctx.editMessageText(`❌ *Error:* Spec "${escapeMarkdown(specKey)}" tidak ditemukan.\n\nSilakan ulangi dari awal: ${config.prefix}buyvps`, {
                parse_mode: "Markdown"
            });
        }
        ...
        const osButtons = Object.entries(vpsImages).map(([osKey, os]) => {
            ...
            return [
                {
                    text: `${os.icon} ${escapeMarkdown(os.name)}${priceText}`,
        ...
        return ctx.editMessageText(`*Paket Terpilih:* ${escapeMarkdown(spec.name)}\n\n*Pilih Operating System:*`, {
            parse_mode: "Markdown",
    ...
    bot.action(/vps_step2\|(.+)/, async (ctx) => {
        ...
        if (!vpsImages[osKey]) {
            return ctx.editMessageText(`❌ *Error:* OS "${escapeMarkdown(osKey)}" tidak ditemukan.\n\nSilakan pilih OS lain.`, {
                parse_mode: "Markdown"
            });
        }
        ...
        const regionButtons = Object.entries(vpsRegions).map(([key, region]) => [
            {
                text: `${region.flag} ${escapeMarkdown(region.name)}`,
        ...
        return ctx.editMessageText(`*Spesifikasi:*\n• ${escapeMarkdown(spec.name)}${additionalCostText}\n• OS: ${escapeMarkdown(osImage.name)}\n\n*Pilih Region Server:*`, {
            parse_mode: "Markdown",
    ...
    bot.action(/vps_step3\|(.+)/, async (ctx) => {
        ...
        if (!vpsRegions[regionKey]) {
            return ctx.editMessageText(`❌ *Error:* Region "${escapeMarkdown(regionKey)}" tidak ditemukan.\n\nSilakan ulangi dari awal: ${config.prefix}buyvps`, {
                parse_mode: "Markdown"
            });
        }
        ...
        if (!regionValidation.valid) {
            return ctx.editMessageText(`❌ *Error:* ${regionValidation.message}\n\nSilakan pilih region lain.`, {
                parse_mode: "Markdown"
            });
        }
    ...
    bot.action(/confirm_vps_payment\|(.+)/, async (ctx) => {
        ...
        if (!vpsSpecs[specKey]) {
            return ctx.reply(`❌ *Error:* Spec "${escapeMarkdown(specKey)}" tidak ditemukan.\n\nSilakan ulangi dari awal: ${config.prefix}buyvps`, {
                parse_mode: "Markdown"
            });
        }

        if (!vpsImages[osKey]) {
            return ctx.reply(`❌ *Error:* OS "${escapeMarkdown(osKey)}" tidak ditemukan.\n\nSilakan ulangi dari awal: ${config.prefix}buyvps`, {
                parse_mode: "Markdown"
            });
        }

        if (!vpsRegions[regionKey]) {
            return ctx.reply(`❌ *Error:* Region "${escapeMarkdown(regionKey)}" tidak ditemukan.\n\nSilakan ulangi dari awal: ${config.prefix}buyvps`, {
                parse_mode: "Markdown"
            });
        }
...
⚠️ Apakah Anda yakin ingin melanjutkan pembayaran?
    ...
    bot.action("back_to_script", async (ctx) => {
        ...
        const scriptButtons = scriptsList.map(s => ([
            {
                text: `📂 ${escapeMarkdown(s.name)} - Rp${Number(s.price).toLocaleString("id-ID")}`,
